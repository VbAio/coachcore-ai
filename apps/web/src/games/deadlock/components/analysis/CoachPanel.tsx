...
export function CoachPanel({ insight, emptyHint }: Props) {
  if (!insight) {
    return (
      <GlassCard className="flex h-full min-h-[420px] items-center justify-center p-6 text-center text-sm text-zinc-500">
        {emptyHint ?? 'Select a timeline moment for a full coaching breakdown.'}
      </GlassCard>
    );
  }

  const wp = insight.winProbabilityDelta ?? insight.impactEstimate?.winProbabilityDelta;
      ...
      <div className="flex-1 space-y-4 overflow-auto px-4 py-4 text-sm">
        ...
        {insight.practiceDrill && (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
            <p className="text-[10px] uppercase tracking-wider text-amber-200/80">Practice drill</p>
